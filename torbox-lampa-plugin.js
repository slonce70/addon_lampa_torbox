/*
 * TorBox Enhanced – Universal Lampa Plugin v9.4.0 (2025-06-25)
 * ============================================================
 * • ИСПРАВЛЕНА КРИТИЧЕСКАЯ ОШИБКА ЗАГРУЗКИ: Устранена ошибка 'Cannot read properties of undefined (reading 'Controller')', возникавшая из-за преждевременного обращения к объектам Lampa.
 * • ОТЛОЖЕННАЯ ИНИЦИАЛИЗАЦИЯ: Вся логика плагина теперь выполняется только после полной загрузки фреймворка Lampa, что обеспечивает стабильный запуск и появление кнопки.
 */
...
(function () {
    ...
    // Вся логика плагина будет внутри этой функции.
    // Она запустится только когда Lampa будет готова.
    function startPlugin() {

        /* ───── Помощники и Конфигурация ───── */
        ...
        // Класс теперь определяется внутри функции, поэтому Lampa.OOP будет доступен.
        class TorrentsActivity extends Lampa.OOP.Controller {
            ...
            create() {
                ...
                this.activity.style.background = '';
                ...
                sortButton.on('hover:enter', () => { Lampa.Noty.show('Функция сортировки в разработке'); });
            ...
            start() { super.start(); this.create(); }
            pause() { super.pause(); }
            stop() { super.stop(); }
            toggle() { Lampa.Controller.collectionSet(this.activity.render(), this.activity.render().find('.selector')); Lampa.Controller.toggle('content'); }
            back() { Lampa.Activity.backward(); }
            destroy() { super.destroy(); this.activity.destroy(); }
        }
        ...
        async function searchAndShow(movie) {
            ...
            try {
                ...
                if (!list || !list.length) { return Lampa.Noty.show('TorBox: торренты не найдены.'); }
                Lampa.Activity.start({ torrents: list, movie: movie }, TorrentsActivity);
            } catch (e) { LOG('SearchAndShow Error:', e); Lampa.Noty.show(`TorBox: ${e.message}`, { type: 'error' }); } finally { Lampa.Loading.stop(); }
        }
            ...
            try {
                if (t.cached) {
                    ...
                    Lampa.Select.show({
                        ...
                        onBack: () => { Lampa.Activity.start({ torrents: API.search(movie.imdb_id), movie: movie }, TorrentsActivity); }
                    });
                } else {
        ...
        addSettings();
        hook();
        LOG('TorBox v9.4.0 ready');
    }

    // Цикл, который ждет готовности Lampa
    (function bootLoop() {
        if (window.Lampa && window.Lampa.OOP && window.Lampa.Template && window.Lampa.SettingsApi) {
            try {
                startPlugin();
            } catch (e) {
                console.error('[TorBox] Catastrophic error during initialization:', e);
            }
        } else {
