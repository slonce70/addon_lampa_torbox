 ...
 * Version 8.0.3 - CRITICAL FIX: Removed recursive loop in settings menu injection.
 *
...
(function () {
    ...
    //--- Инициализация плагина (ОКОНЧАТЕЛЬНО ИСПРАВЛЕННАЯ ВЕРСИЯ) ---
    function init() {
        Lampa.Settings.listener.follow('open', (e) => {
            if (e.name === 'main' && !e.body.find('[data-action="torbox_settings"]').length) {
                ...
                // Вставляем кнопку после пункта "Плагины" (или "Расширения")
                e.body.find('[data-action="plugins"]').after(button);
                
                // КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: УДАЛЕНА СТРОКА, ВЫЗЫВАЮЩАЯ РЕКУРСИЮ
                // Lampa.Controller.enable('settings'); // <<-- ЭТА СТРОКА ВЫЗЫВАЛА ОШИБКУ
            }
        });
        ...
        console.log(`%c${PLUGIN_NAME} v8.0.3`, 'color: #2E7D32; font-weight: bold;', '– плагин успешно загружен, критическая ошибка в настройках исправлена.');
    }
