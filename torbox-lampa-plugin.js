...
(function(){  'use strict';
  ...
  const API={
    ...
    async main(path,body={},method='GET'){
      ...
      else if(method!=='GET'){opt.headers='application/json';opt.body=JSON.stringify(body);} 
    ...
    async search(term){
      ...
      // КОРРЕКЦИЯ: Используем заголовок 'X-Api-Key' для btm.tools API
      // Предыдущая реализация использовала 'Authorization: Bearer', что приводило к 403 Forbidden.
    ...
    files(id){return this.main('/torrents/mylist',{id}).then(r=>r.data?.?.files||);},
  ...
  async function handleTorrent(t,movie,all){
    ...
    try{
      if(t.cached){
        ...
        if(vids.length===1){play(t.id,vids,movie);return;}
  ...
  let t=0;const WAIT=500,MAX=60000;(function loop(){
    if(window.Lampa&&window.Lampa.Settings){try{addSettings();hook();LOG('Ready v3.0.3');}catch(err){console.error('',err);}return;}
    if((t+=WAIT)>=MAX){console.warn(' Lampa not found');return;}
