...
const Api = (() => {
    ...
    // Глобальный AbortController для отмены всех запросов
    const abortHandlers = new Set(); // Для отслеживания обработчиков
    ...
    const request = async (url, opt = {}, signal) => {
        ...
        // Создаем обработчики для правильной очистки
        const signalHandler = () => controller.abort();
        const globalHandler = () => controller.abort();
        
        // Связываем с глобальным контроллером и пользовательским сигналом
        if (signal) {
            signal.addEventListener('abort', signalHandler);
            abortHandlers.add({ signal, handler: signalHandler });
        }
        globalAbortController.signal.addEventListener('abort', globalHandler);
        abortHandlers.add({ signal: globalAbortController.signal, handler: globalHandler });
        ...
        try {
            ...
            if (e.name === 'AbortError') {
                if (globalAbortController.signal.aborted) {
                    LOG('Request cancelled by global abort');
                    return null;
                }
            ...
            if (signal) {
                ...
                abortHandlers.delete({ signal, handler: signalHandler });
            }
            globalAbortController.signal.removeEventListener('abort', globalHandler);
            abortHandlers.delete({ signal: globalAbortController.signal, handler: globalHandler });
        }
    };
    ...
    return { 
        ...
        cancelAllRequests() {
            ...
            LOG('All API requests cancelled');
        },
        destroy() {
            globalAbortController.abort();
            LOG('API module destroyed');
        }
    };
})();
...
function MainComponent(object) {
    ...
    // Добавляем debounced версию search для предотвращения race conditions
    const debouncedSearch = Utils.debounce((force, customTitle) => {
        searchInternal(force, customTitle);
    }, 300);
    
    const searchInternal = (force = false, customTitle = null) => {
    ...
    const track = (id, signal) => {
        return new Promise((resolve, reject) => {
            ...
            const poll = async () => {
                ...
                try {
                    const response = await Api.myList(id, signal);
                    if (!response || !response.data) {
                        if (active) {
                            pollTimeoutId = setTimeout(poll, 10000);
                        }
                        return;
                    }
                    ...
                    if (is_finished && d.files?.length) {
                        ...
                        const perc = (parseFloat(d.progress) * 100) || 0;
                        const speed = Utils.formatBytes(d.download_speed || 0, true);
                        const eta = Utils.formatTime(d.eta || 0);
    ...
    const drawEpisodes = (torrent_data) => {
        ...
        // Улучшенное управление фокусом для эпизодов
        if (last_focused_element && last_focused_element.length) {
            ...
            // Если нет последнего воспроизведенного, фокусируемся на первом элементе
            const firstElement = scroll.render().find('.selector').first();
            if (firstElement.length) {
                last = firstElement[0];
                setTimeout(() => {
                    if (last && scroll) {
                        scroll.update(firstElement, true);
                    }
                }, 50);
            }
        }
    ...
    this.draw = function (items) {
        ...
        if (focus_element && focus_element.length) {
            ...
            // Принудительно обновляем скролл для корректного позиционирования
            setTimeout(() => {
                if (last && scroll) {
