/*
 * TorBox Enhanced – Universal Lampa Plugin v26.0.0 (Hybrid UI & API Engine)
 * =================================================================================
 * • ВИПРАВЛЕННЯ ІНТЕРФЕЙСУ ТА НАВІГАЦІЇ: Повністю відновлено стабільний UI на основі
 * нативних компонентів Lampa.Explorer та Lampa.Filter, що вирішує проблеми з фокусом,
 * прокруткою та візуальним відображенням списку.
 * • ЗБЕРЕЖЕННЯ НОВОЇ ЛОГІКИ API: Інтегровано сучасний механізм пошуку по публічним
 * парсерам, перевірки кешу та взаємодії з API з версії v25.
 * • ОПТИМІЗОВАНО СТРУКТУРУ: Код компонента реорганізовано для кращої
 * читабельності та підтримки, поєднуючи найкращі практики з обох версій.
 */
...
(function () {
  ...
  /* ───── Guard double-load ───── */
  const PLUGIN_ID = 'torbox_enhanced_v26_0_0_hybrid';
  ...
  const Cache = {
      ...
      get: function(key) {
          ...
          if (Date.now() - entry.timestamp > FIVE_MINUTES) {
              ...
              LOG(`Локальний кэш для ключа '${key}' застарів і був видалений.`);
              return null;
          }
  ...
  if (!$('#torbox-component-styles').length) {
    $('head').append(`<style id="torbox-component-styles">
        ...
        .torrent-list{padding:1em} /* from v18 */
        ...
        .torbox-status__info{font-size:1.1em;margin-bottom:.8em;color:var(--color-text)}
        ...
        .torbox-status__progress-bar{height:100%;width:0%;background:linear-gradient(90deg,var(--color-primary),var(--color-primary-light,#4CAF50));transition:width .5s ease-out;border-radius:8px;position:relative}
        .torbox-status__progress-bar::after{content:'';position:absolute;top:0;left:0;right:0;bottom:0;background:linear-gradient(45deg,transparent 30%,rgba(255,255,255,.2) 50%,transparent 70%);animation:shimmer 2s infinite}
        @keyframes shimmer{0%{transform:translateX(-100%)}100%{transform:translateX(100%)}}
        .modal .torbox-status__progress-container{background:rgba(255,255,255,.2)!important}
    ...
    function TorBoxComponent(object) {
        ...
        // State initialization based on v18 for Lampa components
        this.state = {
            scroll: null,
            files: null,
            filter: null,
    ...
    // UI rendering using Lampa.Explorer (from v18 logic)
    TorBoxComponent.prototype.create = function() {
        LOG("Component create() with Lampa.Explorer");
        this.state.scroll = new Lampa.Scroll({ mask: true, over: true });
        this.state.files = new Lampa.Explorer(this.activity.object);
        this.state.filter = new Lampa.Filter(this.activity.object);
    ...
    // Controller setup (from v18 logic)
    TorBoxComponent.prototype.start = function() {
        ...
        Lampa.Controller.add('content', {
            toggle: () => {
                ...
                Lampa.Controller.collectionFocus(this.state.last_focused, this.state.scroll.render());
            },
    ...
    TorBoxComponent.prototype.loadData = async function(force_refresh = false) {
        ...
        try {
            ...
            this.empty(error.message || 'Ошибка при загрузке данных');
        } finally {
            this.activity.loader(false);
        }
    };
    
    // Display orchestration (from v18)
    TorBoxComponent.prototype.display = function() {
        this.updateFilterUI();
        this.draw(this.applyFiltersAndSort());
    };
    
    // Logic for applying filters (from v25 and v18)
    ...
    // Drawing the list (from v18, item template from v25)
    TorBoxComponent.prototype.draw = function(torrents_list) {
        ...
        torrents_list.forEach(t => {
            ...
            itemHtml.on('hover:focus', () => {
                ...
                this.state.scroll.update(itemHtml, true);
            });
            itemHtml.on('hover:enter', () => {
                handleTorrent(t, this.movie, this, this.abortController.signal);
            });
            this.state.scroll.append(itemHtml);
        });
        
        // This is crucial for initial focus
        Lampa.Controller.collectionFocus(this.state.last_focused, this.state.scroll.render());
    };
    
    // Filter UI setup (from v18)
    TorBoxComponent.prototype.initializeFilterHandlers = function() {
        this.state.filter.onSelect = (type, a, b) => {
            Lampa.Select.close();
            
            if (type === 'sort') {
                this.state.sort = a.value;
                Store.set('torbox_sort_method', a.value);
                this.display();
            }
    ...
    TorBoxComponent.prototype.updateFilterUI = function() {
        ...
        ].map(item => ({...item, selected: item.value === sort}));

        filter.set('sort', sort_items);
        filter.render().find('.filter--sort span').text('Сортировка');
        filter.chosen('sort', [ (sort_items.find(s => s.selected) || {title:''}).title ]);
        ...
        const filter_items = [
            ...
            {title:'Обновить (очистить кэш)', refresh: true}
        ];
    ...
    TorBoxComponent.prototype.empty = function(message) {
        this.state.scroll.clear();
        this.state.scroll.append(`<div class="empty selector">${escapeHtml(message)}</div>`);
        Lampa.Controller.collectionSet(this.state.scroll.render());
        Lampa.Controller.collectionFocus(false, this.state.scroll.render());
    };
    ...
    // MODAL, PLAYER, AND TORRENT HANDLING LOGIC FROM V25 (UNCHANGED)
    let modalCache = {};
    ...
    addSettings();
    boot();
    LOG('TorBox v26.0.0 (Hybrid Engine) ready');
  }
