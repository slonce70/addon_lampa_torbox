...
(function(){
  ...
  // ───────────────────────────── 2. TorBox API ────────────────────────────────
  const API={
    // choose proxy to bypass CORS if browser blocks Authorization header
    _proxy(url){
      return `https://corsproxy.io/?url=${encodeURIComponent(url)}`; // no account needed for low‑volume
    },
    ...
    async _req(key,endpoint,qs={},method='GET',base=this.BASE){
      ...
      if(typeof window==='object' && window.location){
        // Browser context: always proxy to handle CORS
        url=this._proxy(url);
      }
