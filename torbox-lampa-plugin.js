 ...
 *
 * Changelog v51.0.4:
 * - FIX (CRITICAL): Исправлена логика отмены загрузки. Теперь индикатор загрузки корректно закрывается при нажатии кнопки "Назад" или клике мимо, и интерфейс больше не зависает.
 *
 * Changelog v51.0.3:
 * - FIX (CRITICAL): Восстановлена оригинальная логика вставки кнопки "TorBox" на страницу фильма.
 *
 * Changelog v51.0.2:
 * - FIX (Robustness): Улучшена совместимость и очистка памяти.
 * - FIX (Debug): Улучшено логирование.
 */
(function () {
    'use strict';
    ...
    function MainComponent(object) {
        ...
        const onTorrentClick = (torrent) => {
            ...
            // FIX: Исправлена логика обработки ошибок для корректной отмены
