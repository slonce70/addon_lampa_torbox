/*
 * TorBox Enhanced – Universal Lampa Plugin v25.0.0 (Architecture Refactoring)
 * =================================================================================
 * • ПОЛНЫЙ РЕФАКТОРИНГ АРХИТЕКТУРЫ: Компонент переписан с использованием
 * современного синтаксиса ES6-классов и четкого разделения ответственности
 * методов для максимальной стабильности и читаемости.
 * • НАДЕЖНАЯ НАВИГАЦИЯ: Реализован архитектурный паттерн с немедленной
 * отрисовкой "скелета" UI и явным управлением Lampa.Controller, что полностью
 * решает проблему сбоя навигации после асинхронной загрузки.
 * • УПРОЩЕННОЕ УПРАВЛЕНИЕ UI: Заменена сложная связка Lampa.Explorer/Filter
 * на прямое управление DOM, что дает полный контроль над рендерингом и
 * предотвращает потенциальные конфликты.
 * • СОХРАНЕНИЕ ФУНКЦИОНАЛА: Вся логика API (поиск, проверка кеша, добавление
 * торрентов, отслеживание, запуск плеера) сохранена и интегрирована в новую
...
(function () {
  ...
  /* ───── Guard double-load ───── */
  const PLUGIN_ID = 'torbox_enhanced_v25_0_0_refactored';
  ...
  /**
   * TorBoxComponent (v25.0) - ES6 Class Refactoring
   * Компонент переписан с использованием современного синтаксиса для улучшения
   * структуры, читаемости и надежности.
   */
  class TorBoxComponent extends Lampa.Activity {
    ...
    /**
     * Шаг 1: Синхронное создание UI-скелета.
     * Выполняется мгновенно, не содержит асинхронных операций.
     */
    create() {
        ...
        // Создание базовой разметки и компонента Lampa.Scroll
    ...
    /**
     * Шаг 2: Запуск асинхронных операций и активация контроллера.
     */
    start() {
        super.start();
        LOG("Component start()");

        // Активируем Lampa.Controller немедленно
        Lampa.Controller.add('content', {
            ...
            back: () => {
                // Обработка кнопки "назад" для закрытия модальных окон или выхода
        ...
        // Рисуем элементы управления (фильтры, сортировка)
        this.renderHead();

        // Запускаем основную логику загрузки, НЕ дожидаясь ее завершения
        this.loadData();
    }
    
    /**
     * Шаг 3: Асинхронная загрузка и обработка данных.
     */
    async loadData(force_refresh = false) {
    ...
    /**
     * Шаг 4: Отрисовка контента (фильтрация, сортировка, рендеринг).
     */
    renderContent() {
        ...
        // Ключевой шаг: Обновляем Lampa.Controller после изменения DOM
    ...
    /**
     * Отрисовка "шапки" с кнопками управления.
     */
    renderHead() {
    ...
    /**
     * Отображает сообщение о статусе (загрузка, ошибка, пусто).
     */
    ...
    /**
     * Явно обновляет Lampa.Controller.
     */
    updateNavController() {
        Lampa.Controller.collectionSet(this.activity.render());
        Lampa.Controller.collectionFocus(this.state.last_focused, this.activity.render());
    ...
    /**
     * Шаг 5: Полная очистка ресурсов при уничтожении компонента.
     */
    destroy() {
        super.destroy();
  ...
  /* ───── Logic for modals, player, etc. (mostly unchanged) ───── */
  let modalCache = {};
  ...
  (function bootLoop () {
    if (window.Lampa && Lampa.Activity) {
      try {
        ...
        addSettings();
        boot();
        LOG('TorBox v25.0.0 (Architecture Refactoring) ready');
      }
