 ...
 * ============================================================
 *  ▸ Исправляет ошибку «NO_AUTH» — search‑api.torbox.app принимает токен
 *    только в заголовке **X‑Api-Key** (без «Bearer»). Запрос прокидывается
 *    через corsproxy.io, чтобы обойти CORS.
 *  ▸ Полный, самодостаточный код без сокращений и «оставьте прежнее».
 */

(function(){
  'use strict';

  const PLUGIN_ID = 'torbox_enhanced_v3_0_7';
  if (window[PLUGIN_ID]) return;               // защита от двойной загрузки
  ...
  const CFG = {
    get apiKey(){       return Store.get('torbox_api_key', ''); },
    set apiKey(v){      Store.set('torbox_api_key', v.trim()); },
    get debug(){        return Store.get('torbox_debug', 'false') === 'true'; },
    set debug(v){       Store.set('torbox_debug', !!v); },
    get cachedOnly(){   return Store.get('torbox_cached_only', 'false') === 'true'; },
    set cachedOnly(v){  Store.set('torbox_cached_only', !!v); }
  };
  const LOG = (...a) => CFG.debug && console.log('[TorBox]', ...a);

  const CORS    = u        => `https://corsproxy.io/?url=${encodeURIComponent(u)}`;
  const withKey = (u, key) => `${CORS(u)}&header_X-Api-Key=${encodeURIComponent(key)}`;
  const ql = n => { n = n.toLowerCase(); if(/(2160|4k)/.test(n)) return '4K'; if(/1080/.test(n)) return '1080p'; if(/720/.test(n)) return '720p'; return ''; };

  // ─────────────────────────── 2. Работа с API TorBox ─────────────────────────
  const API = {
    SEARCH_BASE: 'https://search-api.torbox.app/torrents/search/',
    MAIN       : 'https://api.torbox.app/v1/api',

    async search(term){
      if(!CFG.apiKey) throw new Error('TorBox: нужен API‑Key для поиска');
      const safe = encodeURIComponent(term).replace(/%3A/ig, ':');
      const raw  = `${this.SEARCH_BASE}${safe}?metadata=true&check_cache=true`;
      const url  = withKey(raw, CFG.apiKey);
      LOG('Search URL', url);
      const r = await fetch(url);
      if(!r.ok){ const txt = await r.text().catch(()=>r.statusText); throw new Error(`TorBox: ${txt || r.status}`); }
      return r.json();
    },
  ...
  function addSettings(){
    if(Lampa.SettingsApi){
      ...
      params.forEach(p => Lampa.SettingsApi.addParam({
        component: COMP,
      ...
      // Старый Lampa (без SettingsApi)
      const folder = `<div class="settings-folder selector" data-component="${COMP}"><div class="settings-folder__icon">${ICON}</div><div class="settings-folder__name">TorBox Enhanced</div></div>`;
      Lampa.Settings.main().render().find('[data-component="more"]').after($(folder));
      const tpl = 'settings_' + COMP;
      if(!Lampa.Template.get(tpl)) Lampa.Template.add(tpl,
        `<div class="torbox-set">
           <div class="settings-param selector" data-k="key">API‑Key <span></span></div>
           <div class="settings-param selector" data-k="cached">Только кеш <span></span></div>
           <div class="settings-param selector" data-k="dbg">Debug <span></span></div>
         </div>`);
  ...
  function hook(){
    Lampa.Listener.follow('full', e => {
      ...
      if(root.find('.view--torbox').length) return; // уже добавлено
  ...
  // ─────────────────────────── 6. Boot‑loop ────────────────────────────────
  let waited = 0; const STEP = 500, MAX = 60000;
  (function loop(){
    if(window.Lampa && window.Lampa.Settings){
      try { addSettings(); hook(); LOG('Ready v3.0.7'); } catch(err){ console.error('[TorBox]', err); }
      return;
    }
    if((waited += STEP) >= MAX){ console.warn('[TorBox] Lampa not found'); return; }
