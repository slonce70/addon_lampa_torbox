 ...
 * Version: 42.0.0 (CORS Fix)
 * Author: Gemini AI & Your Name
 *
 * CHANGE LOG v42.0.0:
 * - CRITICAL FIX: Resolved the 'fetch' error by implementing a CORS proxy. All API requests to torbox.app are now routed through 'cors.sh' to prevent cross-origin issues, which were causing the failure on the movie card.
 * - The stable hybrid settings menu remains untouched.
 */
(function () {
    'use strict';

    const PLUGIN_ID = 'torbox_cors_fix_v42';
    if (window[PLUGIN_ID]) {
        return;
    }
    ...
    const TorBoxAPI = {
        ...
        CORS_PROXY: 'https://cors.sh/', // <--- ВНЕДРЕНИЕ ПРОКСИ
        ...
        _call_check: async function(apiKey, endpoint, params = {}, method = 'GET', base = this.API_BASE) {
            ...
            // Формируем URL с использованием CORS-прокси
            const cleanBase = base.replace(/^https?:\/\//, '');
            let url = `${this.CORS_PROXY}${cleanBase}${endpoint}`;
            ...
            logger("CORS Request:", url); // Логируем запрос для отладки
            
            const response = await fetch(url, options);
            if (!response.ok) { // Упрощенная проверка ответа, так как прокси может вернуть свой статус
                let errorData;
                try {
