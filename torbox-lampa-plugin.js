...
.modal .torbox-status__progress-bar{background:linear-gradient(90deg, #4CAF50, #66BB6A) !important;}
  ...
  function TorBoxComponent(object) {
    ...
    this.updateFilterUI = function() {
        ...
        // Ensure all_torrents is an array before mapping
...
```Гаразд, я уважно вивчив ваш код і логіку. Вибраний фрагмент є повним кодом плагіна, і я бачу, що ви вже застосували деякі з попередніх виправлень. Однак, щоб вирішити останню проблему з `all_torrents.map is not a function`, потрібно внести фінальні корективи.

### План

1.  **Основна проблема:** Функція `API.search` повертає весь об'єкт відповіді від сервера (`json`), а не масив торентів (`json.data.torrents`), який знаходиться всередині.
2.  **Рішення:** Я виправлю функцію `API.search`, щоб вона завжди повертала масив — або масив торентів, якщо запит успішний, або порожній масив `[]` у разі помилки чи відсутності даних.
3.  **Додаткова надійність:** Я додам ще одну перевірку безпосередньо перед використанням даних, щоб гарантувати, що змінна `all_torrents` є масивом, що зробить код стійкішим.

Я зараз внесу ці зміни у ваш код у Canvas.


```javascript
/*
 * TorBox Enhanced – Universal Lampa Plugin v11.0.41 (Data Handling Fix)
 * ============================================================
...
(function () {
  ...
  /* ───── Guard double-load ───── */
  const PLUGIN_ID = 'torbox_enhanced_v11_0_41_datafix';
  if (window[PLUGIN_ID]) return;
  window[PLUGIN_ID] = true;
...
.modal .torbox-status__progress-bar{background:linear-gradient(90deg, #4CAF50, #66BB6A) !important;}
  ...
  function TorBoxComponent(object) {
    ...
    this.updateFilterUI = function() {
        ...
        // ### FIXED ### - Add safeguard here as well
        if (!Array.isArray(all_torrents)) all_torrents = [];
    ...
    this.draw = function(torrents_list) {
        scroll.clear();
        if (!torrents_list || !torrents_list.length) { // Added check for torrents_list itself
            this.empty('Ничего не найдено по заданным фильтрам');
            return;
        }
        torrents_list.forEach(t => {
