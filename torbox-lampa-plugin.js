    ...
    function startPlugin() {
        ...
        async function searchAndShow(movie) {
            ...
            try {
                ...
                if (!filteredTorrents.length) return Lampa.Noty.show('Нет кэшированных результатов. Проверьте настройки плагина.');
                    ...
                    if (videos.length === 1) {
                        ...
                        Lampa.Select.show({
                            ...
                            items: videos.map(f => {
                                ...
                                return { title: f.name, subtitle: [quality, size].filter(Boolean).join(' | '), tid: torrent.id, fid: f.id };
                            }),
        ...
        async function playFile(torrentId, fileId, movie) {
            ...
            try {
                ...
                Lampa.Player.play({ url: downloadUrl, title: movie.title, poster: movie.img });
