...
- `API key`: личный ключ TorBox. Он обязателен для проверки кэша, добавления торрентов и запуска файлов.
...
### 51.2.5
- TorBox API-запросы теперь отправляют актуальный `Authorization: Bearer` вместе с совместимым `X-Api-Key`, чтобы `checkcached/mylist/requestdl` не зависали на auth-fail через прокси.
- Убран fallback на общий вшитый API-ключ: live-проверка показала `Not authenticated`, поэтому теперь плагин честно требует личный ключ вместо долгих ложных ожиданий.
- `checkcached` больше не проглатывает ошибки авторизации/валидации как будто это просто «нет кэша».
- Трекинг загрузки переживает единичный сетевой/timeout сбой `mylist` и продолжает polling вместо падения через один 20-секундный запрос.
