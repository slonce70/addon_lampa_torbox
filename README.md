...
- Торрент с несколькими файлами: вход в эпизоды -> навигация -> OK на серии запускает просмотр -> Back -> возврат.
- В списке эпизодов: `Right` с серии -> кнопка `⬇`, OK на `⬇` получает прямую ссылку TorBox, передает её системе для загрузки/внешнего открытия и копирует ссылку как fallback; `Left` возвращает на серию.
