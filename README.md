# TorBox Lampa Plugin (Stable Refactored)

Надежный плагин на чистом JavaScript, который легко интегрирует облачный торрент-клиент [TorBox.app](https://torbox.app) с медиацентром [Lampa](https://lampa.mx). Смотрите торренты мгновенно, используя кэшированный контент TorBox, или загружайте их по запросу — всё в привычном интерфейсе Lampa.

> **Версия 51.1.2 (Стабильная)**
>
> Релиз сфокусирован на автономности плагина и контроле за поведением интерфейса:
>
> - **Горячий фикс:** настройки трекинга и расширений больше не ломают загрузку — вспомогательные функции доступны до инициализации и защищены от ошибок.
> - **Навигация без сюрпризов:** фокус теперь перемещается по явной матрице маршрутов, учитывая панель «Продолжить просмотр» и переключатель кеша. При возврате из подменю позиция восстанавливается без ручных корректировок.
> - **Устойчивый старт воспроизведения:** UI автоматически откатывается, если воспроизведение отменено или TorBox вернул 422. Отметка просмотренных серий накладывается только после фактического старта.
> - **Легковесное состояние:** «сырые» данные больше не попадают в кеш и `state`. Списки просмотренных серий обрезаются до 250 элементов, снапшоты торрентов нормализуются и обрезаются по размеру.
> - **Расширяемые настройки:** добавлены параметры для количества попыток трекинга, интервала опроса и списка видео-расширений. Ввод прокси и API-ключа нормализуется, исключая скрытые символы.
> - **Умнее поиск:** генератор уточняющих комбинаций учитывает альтернативные локализации, ключевые слова, сезоны и идентификаторы, а подсказки фильтра отображают категорию каждого варианта. В debug-режиме выводятся сгенерированные комбинации и метрики списка.

## ✨ Ключевые Возможности

### 🎬 Умный Поиск Контента
- **Интеллектуальный поиск:** Поиск торрентов по нескольким публичным трекерам (Viewbox, Jacred) с автоматическим переключением.
- **Пользовательские запросы:** Уточняйте выдачу готовыми комбинациями (альтернативные названия, годы, сезоны/эпизоды, ключевые слова и идентификаторы). В списке подсказок отображаются метки, показывающие происхождение варианта.
- **Приоритет кэша:** Мгновенное определение доступных для стриминга торрентов с иконкой `⚡`.
- **Распознавание качества:** Автоматическое определение качества (4K/FHD/HD/SD) по названию торрента.

### 🎯 Продвинутая Фильтрация и Сортировка
- **Многоуровневая фильтрация:** Фильтруйте по качеству, типу видео (HDR/DV), языку аудио, кодекам и трекерам.
- **Умный переключатель кэша:** В один клик переключайтесь между отображением только кэшированных (`⚡`) и всех (`☁️`) торрентов.
- **Гибкая сортировка:** Сортируйте по сидам, размеру файла или дате добавления с сохранением выбора.
- **Динамические фильтры:** Опции фильтрации генерируются на лету на основе доступных торрентов.

### 🎮 Улучшенный Пользовательский Опыт
- **Продолжить просмотр:** Специальная панель для быстрого доступа к последнему запущенному торренту. Снапшоты компактные и автоматически откатываются, если воспроизведение не стартовало.
- **Информативный статус загрузки:** Отображается статус "Обработка", когда торрент скачан, но еще не готов к воспроизведению.
- **Менеджер эпизодов:** Прогресс фиксируется только после успешного запуска, списки просмотренных серий нормализуются и не разрастаются бесконечно.
- **Память фокуса:** Явная таблица маршрутов учитывает панель «Продолжить просмотр» и переключатель кеша, поэтому положение курсора предсказуемо.
- **Оптимизация для пульта:** Полная поддержка навигации с пульта телевизора с предсказуемым поведением фокуса.

### 🛡️ Повышенная Стабильность
- **Надежная обработка ошибок:** Комплексная защита от ошибок с информативными сообщениями для пользователя.
- **Безопасное управление состоянием:** Защита от поврежденных данных в `localStorage`.
- **Изоляция ошибок рендеринга:** Ошибка в одном элементе списка не нарушит отображение остальных.
- **Эффективное кэширование:** LRU-кэш с автоматической очисткой для снижения нагрузки на сеть.
- **Устойчивость к сети:** Обработка таймаутов, логика отмены запросов и понятные сообщения о сетевых сбоях.

### ⚙️ Настройки и диагностика
- **Контроль трекинга:** Настройте количество попыток проверки статуса торрента и интервал между опросами (диапазоны 3–120 и 3000–60000 мс). Значения автоматически нормализуются.
- **Гибкий список расширений:** Управляйте фильтрацией эпизодов через отдельное поле для видео-расширений, пробелы и регистр не имеют значения.
- **Диагностика в debug-режиме:** В консоли отображаются размер текущего списка, объем снапшота и набор сгенерированных комбинаций для уточнения поиска.

## 🚀 Быстрый Старт

### Установка
1.  Откройте Lampa и перейдите в `Настройки` → `Плагины`.
2.  Нажмите `Добавить плагин`.
3.  Введите URL плагина:
    ```
    [https://slonce70.github.io/addon_lampa_torbox/torbox-lampa-plugin.js](https://slonce70.github.io/addon_lampa_torbox/torbox-lampa-plugin.js)
    ```
4.  Нажмите `Enter` для установки.
5.  На страницах фильмов и сериалов появится кнопка **TorBox**.

### Начальная Настройка
1.  Перейдите в `Настройки` → `TorBox`.
2.  Заполните обязательные поля:
    - **API-Key**: Ваш ключ API от TorBox.app (можно получить в [панели управления TorBox](https://torbox.app/settings)).
    - **URL CORS-прокси**: URL-адрес CORS-прокси.
    - **Debug-режим**: Включите для диагностики проблем (опционально).
3.  При необходимости настройте дополнительные параметры:
    - **Повторы отслеживания** (3–120): Сколько попыток сделать, прежде чем считать торрент "неподтверждённым" (по умолчанию 24).
    - **Интервал отслеживания (мс)** (3000–60000): Пауза между запросами статуса, влияет на скорость и нагрузку (по умолчанию 10000).
    - **Видео-расширения**: Список форматов через запятую, который используется для отбора файлов серии (`mkv,mp4,avi,ts,m4v,webm` по умолчанию).

## 🔧 Устранение Неполадок

### Распространенные Проблемы

#### "CORS-proxy не задан в настройках"
-   **Причина**: Отсутствует или неверно указан URL CORS-прокси.
-   **Решение**: Укажите корректный URL в настройках плагина.

#### "401 – неверный API-ключ"
-   **Причина**: Недействительный или просроченный API-ключ TorBox.
-   **Решение**: Сгенерируйте новый ключ в панели управления TorBox и введите его в настройках.

#### "Все публичные парсеры недоступны"
-   **Причина**: Публичные трекеры временно не работают или заблокированы вашим провайдером.
-   **Решение**: Попробуйте позже или проверьте сетевое соединение.

### Режим Отладки
Включите режим отладки в настройках, чтобы видеть подробные логи в консоли браузера:
1.  Нажмите `F12` на клавиатуре, чтобы открыть инструменты разработчика.
2.  Перейдите на вкладку `Console`.
3.  Ищите сообщения с префиксом `[TorBox]`.

В debug-логах дополнительно выводятся:
- количество элементов в текущем списке и его фильтрах;
- размер последнего сохранённого снапшота и количество «сырых» записей в кеше;
- набор сгенерированных комбинаций для уточнения поиска.

## 🧾 Changelog

### 51.1.2
- Добавлена защита `try/catch` вокруг инициализации, запись стека ошибки в `localStorage` и нотификация в Lampa, обновление версии.

### 51.1.1
- Исправлен критический регресс, мешавший загрузке плагина (инициализация контроллеров фокуса вызывалась до определения функций).

### 51.1.0
- Введена табличная схема фокуса с учётом панели «Продолжить просмотр» и переключателя кеша.
- Добавлен безопасный откат UI при отменённом/несостающемся воспроизведении, отметка серий выполняется только после старта.
- Исключены «сырые» данные из кеша и состояния, списки просмотренных серий ограничены 250 записями, снапшоты урезаются по размеру.
- Появились настройки количества повторов трекинга, интервала опроса и списка видео-расширений; данные нормализуются автоматически.
- Генератор уточнений поиска учитывает альтернативные названия, годы, сезоны, ключевые слова и ID, подсказки снабжены метками категорий.
